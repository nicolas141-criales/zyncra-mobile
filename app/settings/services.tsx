  ...
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <SafeAreaView style={{ flex: 1, backgroundColor: Colors.cream2 }}>
        <LinearGradient colors={Gradients.ink} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.mHeader}>
...
export default function ServicesScreen() {
  ...
  const load = async () => {
    ...
    (appts ?? []).forEach(a => { if (a.service_id) countMap[a.service_id] = (countMap[a.service_id] ?? 0) + 1; });
            ...
            <Animated.View key={svc.id} entering={FadeInRight.delay(i * 50).duration(320)}>
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={s.info}>{svc.duration_min} min{svc.apptCount ? ` · ${svc.apptCount} citas` : ""}</Text>
